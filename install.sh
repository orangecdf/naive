...
list_nodes() {
  ...
  for config in "${NAIVE_CONFIGS}"naive-*.json; do
    ...
    local port user domain
    port=$(grep -oP '"listen":\s*\["\s*:\K\d+' "$config" | head -1)
    local meta="${NAIVE_CONFIGS}naive-${id}.meta"
    if [[ -f "$meta" ]]; then
      user=$(grep '^username=' "$meta" | cut -d= -f2)
      password=$(grep '^password=' "$meta" | cut -d= -f2)
      domain=$(grep '^domain=' "$meta" | cut -d= -f2)
    else
      user=$(grep -oP '"auth_user_deprecated":\s*"\K[^"]+' "$config" | head -1)
      domain=$(grep -oP '"sni":\s*\["\K[^"]+' "$config" | head -1)
      [[ -z "$user" ]] && user="(未知)"
    fi
    local status_color
    local status_text
    ...
    echo_content white   "  │  端口:   ${port}"
    echo_content white   "  │  用户名: ${user}"
    echo_content white   "  │  密码:   ${password}"
    echo_content white   "  │  域名:   ${domain}"
    echo_content ${status_color} "  └─ ${status_text}"
    echo ""
  done
  if [[ $found -eq 0 ]]; then
    echo_content yellow "  (暂无节点)"
  fi
}
...
# 安装 na 快捷命令
install_shortcut() {
  local fixed_path="/usr/local/naive/install.sh"
  local raw_url="https://raw.githubusercontent.com/orangecdf/naive/main/install.sh"

  if [[ ! -f "$fixed_path" ]]; then
    mkdir -p /usr/local/naive
    echo_content green "---> 下载脚本到 ${fixed_path}"
    curl -fsSL "$raw_url" -o "$fixed_path" && chmod +x "$fixed_path"
  fi

  cat >/usr/local/bin/na <<'EOF'
#!/usr/bin/env bash
bash /usr/local/naive/install.sh "$@"
EOF
  chmod +x /usr/local/bin/na
  echo_content skyBlue "---> 快捷命令 'na' 已安装，直接输入 na 调出菜单"
}
...
add_node() {
  ...
  # 保存明文元数据供列表显示
  cat >"${NAIVE_CONFIGS}naive-${node_id}.meta" <<EOF
...
EOF
  ...
  echo ""
  echo_content green  "  ┌─────────────────────────────"
  echo_content skyBlue "  │  节点 #${node_id} 添加成功！"
  echo_content white   "  │  端口:   ${naive_port}"
  echo_content white   "  │  用户名: ${naive_username}"
  echo_content white   "  │  密码:   ${naive_password}"
  echo_content white   "  │  域名:   ${naive_domain}"
  echo_content green  "  └─────────────────────────────"
}
...
remove_node() {
  ...
  rm -f "${NAIVE_CONFIGS}naive-${node_id}.meta"
...
show_menu() {
  clear
  echo_content red    "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo_content skyBlue " NaiveProxy 多节点管理"
  echo_content red    "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  ...
  while true; do
    ...
    3)
      for config in "${NAIVE_CONFIGS}"naive-*.json; do
        ...
        local id
        id=$(basename "$config" | grep -oP '\d+')
        echo_content skyBlue "━━━ 节点 #${id} 日志 ━━━"
        journalctl -u "naive@${id}" --no-pager -n 20
        echo ""
      done
      read -r -p "按回车返回..." _
      ;;
